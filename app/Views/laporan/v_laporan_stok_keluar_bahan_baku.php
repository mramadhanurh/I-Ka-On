...
<script>
    function ViewTabelLaporan() {
        ...
        if (tgl == "") {
            ...
            $.ajax({
                type: "POST",
                url: "<?= base_url('Laporan/ViewLaporanStokKeluarBahanBaku') ?>",
                data: {
                    tgl: tgl,
                },
                dataType: "JSON",
    ...
    function PrintLaporan() {
        ...
        if (tgl == "") {
            ...
            window.open('<?= base_url('Laporan/PrintLaporanStokKeluarBahanBaku') ?>/' + tgl, 'NewWin', 'toolbar=no, width=1200, height=800, scrollbars=yes');
        }
    }
</script>
